...
export default function GymNameEditor({ gymName, onSave }) {
  ...
  // Update value when gymName changes from outside, but only if not editing
  const [prevGymName, setPrevGymName] = useState(gymName);
  if (gymName !== prevGymName) {
    setPrevGymName(gymName);
    if (!isEditing) {
      setValue(gymName || '');
    }
  }
  ...
  return (
    <span 
      ...
      className="cursor-pointer hover:text-slate-300 transition-colors flex items-center gap-2 group"
